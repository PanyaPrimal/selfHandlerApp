...
class PrivateWorkflowOrderingTests(unittest.TestCase):
    ...
    def test_private_tokens_and_registry_login_never_reach_bundle_scripts(self) -> None:
        ...
        self.assertIn("docker image inspect $env:WEB_IMAGE_REF | ConvertFrom-Json", pull["run"])
        self.assertIn("docker image inspect $env:APP_IMAGE_REF | ConvertFrom-Json", pull["run"])
        self.assertIn("docker image inspect $env:MYSQL_IMAGE_REF | ConvertFrom-Json", pull["run"])
        self.assertIn("[string]$mysqlInspection.Id", pull["run"])
        self.assertNotIn(".RepoDigests", pull["run"])
        self.assertNotIn("--format '{{ index .Config.Labels", pull["run"])
