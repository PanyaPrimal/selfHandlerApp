...
flowchart TD
    ...
    PROFILE --> RECURRENCE[005 Unified recurrence + routine migration]
    PROFILE --> MEASUREMENTS[006 Measurements + body goals]

    AUTH --> STORAGE[007 Storage inbox]
    RECURRENCE --> PLANNER[008 Planner and day planning]
    STORAGE --> PLANNER
    PROFILE --> NOTIFY[009 In-app notifications]
    RECURRENCE --> NOTIFY
    PLANNER --> NOTIFY
    AUTH --> ANDROID[010 Android Capacitor shell]
    NOTIFY --> ANDROID

    RECURRENCE --> HABITS[011 Habits and anti-habits]
    PLANNER --> HABITS
    RECURRENCE --> SLEEP[012 Sleep and routine templates]

    PROFILE --> WORKOUTS[013 Workouts + training goals]
    RECURRENCE --> WORKOUTS
    PLANNER --> WORKOUTS

    PROFILE --> NUTRITION[014 Nutrition]
    MEASUREMENTS --> NUTRITION
    WORKOUTS --> NUTRITION

    PROFILE --> SUPPLEMENTS[015 Supplements]
    RECURRENCE --> SUPPLEMENTS
    NOTIFY --> SUPPLEMENTS

    PROFILE --> LEDGER[016 Finance ledger]
    LEDGER --> FINPLAN[017 Budget + recurring cash flow]
    RECURRENCE --> FINPLAN
    FINPLAN --> FINGOALS[018 Debts + funds + financial goals]
    STORAGE --> FINGOALS

    MEASUREMENTS --> ATTACHMENTS[019 Private attachments]
    NUTRITION --> ATTACHMENTS

    HABITS --> REVIEW[020 Cross-module and periodic review]
    SLEEP --> REVIEW
    WORKOUTS --> REVIEW
    NUTRITION --> REVIEW
    SUPPLEMENTS --> REVIEW
    ...
    REVIEW --> ANALYTICS[021 Analytics and rollups]
    ANALYTICS --> PORTABILITY[022 Data portability]
    RECURRENCE --> CALENDAR[023 Calendar integration]
    PLANNER --> CALENDAR
    ATTACHMENTS --> AI[024 AI assistant, first scenario]
    ANALYTICS --> AI
```
...
### 005 — Unified Recurrence with Routine Migration
...
### 006 — Body Measurements and Body Goals
...
**Prerequisites:** 004. Recurring measurement reminders wait for 009; photos wait for 019.

### 007 — Storage Inbox and Quick Capture
...
the first Capacitor local-notification adapter to the notification contract from 009 when its acceptance
journey is included.

**Prerequisites:** 003 and 009. The shared web flows remain the product source; Android-specific code
...
FCM, and iOS. Camera/gallery arrives with 019; other native capabilities require their own increments.

### 011 — Habits and Anti-Habits
...
**Prerequisites:** 005, 008, and the existing routine/goal baseline. Use 009 only for reminder stories.

### 012 — Sleep and Rich Routine Templates
...
Extend Module 1 without replacing the occurrence source introduced in 005. Feed sleep and routine
aggregates into Today/Review through module-owned summaries.

**Prerequisites:** 004 and 005. Use 008 for day placement and 009 for reminder stories.

### 013 — Workouts and Training Goals
...
### 014 — Nutrition, Meals, Hydration, and Targets
...
**Prerequisites:** 004, 006, and 013.

**Explicitly defer:** photo recognition and receipt-like vision flows until 019 and 024.

### 015 — Supplements, Courses, Intake, and Stock
...
### 017 — Budget and Recurring Cash Flow
...
**Prerequisites:** 005, 009, and 016.

### 018 — Debts, Saving Funds, Financial Goals, and Purchase Links
...
**Prerequisites:** 007, 016, and 017.

### 019 — Private Attachments with First Consumers
...
**Prerequisites:** 006 or 014 plus the existing ownership boundary.

**Explicitly defer:** image recognition, receipt parsing, and GPX parsing.

### 020 — Cross-Module and Periodic Review
...
**Prerequisites:** enough real sources to make the review useful: at minimum 011-015, with Finance
included only after 018.

### 021 — Analytics and Long-Period Rollups
...
**Prerequisites:** 020 and at least two meaningful time-series source modules.

### 022 — Data Portability and Reports
...
**Prerequisites:** 021 for consolidated reports and stable domain contracts for full backup/restore.

### 023 — Calendar Integration
...
**Prerequisites:** 005 and 008. This item may move earlier for product priority after those two are
...
### 024 — AI Assistant Foundation with One Confirmed Scenario
...
019; cross-domain insights require 021.
...
- push/email/Telegram channels: add one adapter per concrete delivery need after 009;
- Strava/Garmin/Apple Health and bank import: add after the corresponding local source of truth is
  stable;
- receipt/meal/body-photo vision and other AI scenarios: add after 019 and 024;
