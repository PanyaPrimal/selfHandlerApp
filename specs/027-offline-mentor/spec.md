...
- Owner clarification (2026-09-20): ordinary online forms save immediately; unavailable-server writes persist locally and send automatically after recovery, without a manual synchronization workflow. Retry while the browser remains online as well as on reconnection/focus/restart. Validation failures preserve input without blocking independent writes. Storage/Planner local projections and dependent IDs retain write order.
- Server-authorized mutations use an operation UUID and request fingerprint. Retry after a lost response returns the original result without repeating side effects. Reusing a UUID with different input fails. Automatic replay applies queued user payloads in order through normal domain validation; the last accepted write wins for supplied fields. Do not gate automatic replay on the global workspace revision, which changes for unrelated records. Legacy revision conflicts retry automatically; invalid drafts remain available for correction. Explicit API clients that supply a base revision still retain the server's precondition check.
